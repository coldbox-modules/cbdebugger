...
component singleton {
	...
	function prettySql( string target = "" ) {
		...
		var indentedKeywords = [ "JOIN", "LEFT JOIN", "INNER JOIN", "OUTER JOIN", "FULL JOIN" ];
		var indent     = "  ";

		return arguments.target
		  .listToArray( variables.NEW_LINE )
		  .map( ( item ) => item.trim() )
		  .map( ( item ) => item.reReplace( "(\s)*,(\s)*", ",#variables.NEW_LINE##indent#", "all" ) )
		  .map( ( item ) => {
			  return item.reReplacenocase( "(\s)*(#keywords.toList( '|' )#)(\s)*", "\2#variables.NEW_LINE##indent#", "all" )
		  })
		  .map( ( item ) => {
			  return item.reReplacenocase( "(#indentedKeywords.toList( '|' )#)", "#variables.NEW_LINE##indent#\1", "all" )
		  })
	...
	string function prettyJson( string target = "" ){
		...
		return arguments.target
			...
			.reReplace( "(\]|\})#variables.NEW_LINE#", "#variables.NEW_LINE#\1", "all" )
			.listToArray( variables.NEW_LINE )
			.map( ( token ) => {
