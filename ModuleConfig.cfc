...
component {
	...
	// App Helpers
	this.applicationHelper  = [ "models/Mixins.cfm" ];
	...
	function configure(){
		variables.interceptorSettings = {
			customInterceptionPoints : [ "beforeDebuggerPanel", "afterDebuggerPanel" ]
		};
	}
	...
	function onLoad(){
		...
		if (
			settings.debugger.showQBPanel && controller
				.getModuleService()
				.isModuleRegistered( "qb" )
		) {
			controller
				.getInterceptorService()
				.registerInterceptor(
