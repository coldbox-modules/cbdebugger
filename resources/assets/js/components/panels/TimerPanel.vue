			...
			<table class="table-auto w-full text-sm text-left text-gray-500 dark:text-gray-200">
				...
				<tbody>
					...
					<tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
						...
						<td class="log-level"><span class="category">{{event.executionTimeMillis}}ms</span></td>
						<td class="log-level"><span class="category">{{event.extraInfo.times}}</span></td>
						<td class="log-level"><span class="category">{{event.extraInfo.type}}</span></td>
						<td class="log-level"><span class="category">{{cleanMethod(event.extraInfo)}}</span></td>
					</tr>
					</template>
				</tbody>
			</table>
...
const cleanMethod = (extraInfo) => {
	if (extraInfo.method) {
		return extraInfo.method.replace(/\[.*\]/, '');
	}
	return '';
}
